...
export default function ExpenseUploadForm() {
  ...
  return (
    <div className="bg-white rounded-2xl sm:rounded-[22px] border border-slate-200/90 shadow-[0_4px_24px_-6px_rgba(0,0,0,0.04)] p-6 sm:p-7 flex flex-col justify-between h-full space-y-4">
      ...
      {message && (
        <div className={`p-3 rounded-xl text-xs flex flex-col gap-0.5 shrink-0 ${message.type === 'success' ? 'bg-orange-50 text-orange-800 border border-orange-200' : 'bg-orange-50 text-orange-800 border border-orange-200'}`}>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col justify-between flex-1 gap-4">
        ...
        <div className="p-3 rounded-xl bg-slate-50 border border-slate-200/80 flex items-center justify-between text-xs">
          <div className="flex items-center gap-2.5 text-slate-700">
            ...
            <div>
              <div className="text-[11px] font-bold text-slate-800 leading-tight">Instant Wallet Settlement</div>
              <div className="text-[10px] text-slate-500 leading-tight">Debited directly from selected wallet balance upon recording.</div>
            </div>
          </div>
          <span className="text-[10px] font-semibold text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded-full border border-emerald-200/80 shrink-0">
            Real-Time
          </span>
        </div>
