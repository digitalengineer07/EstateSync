...
import { CreditCard, Receipt, ShieldCheck, CheckCircle2 } from "lucide-react";
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
            ...
            <div>
              ...
              <span className="text-[11px] text-slate-400 mt-1 block">
                {formData.fundMode === "LIQUID" ? "Direct debit from liquid bank wallet" : "Direct debit from physical petty cash"}
              </span>
            </div>

            <div>
              <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">Expense Date</label>
              <input
                type="date"
                name="date"
                value={formData.date}
            ...
            <div className="sm:col-span-2">
              <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">Description / Purpose</label>
              <input
                type="text"
                name="description"
                ...
                required
                className="w-full text-xs sm:text-sm px-3.5 py-2.5 rounded-xl border border-slate-200 bg-slate-50/70 focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-600 transition text-slate-900 font-medium"
                placeholder="What was this expense for? (e.g. Travel, Client Lunch, Site Tools)"
              />
            </div>
            ...
            <div className="sm:col-span-2">
              ...
              <input
                ...
                onChange={handleChange}
                className="w-full text-xs sm:text-sm px-3.5 py-2.5 rounded-xl border border-slate-200 bg-slate-50/70 focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-600 transition text-slate-900 font-medium font-mono"
                placeholder="e.g. INV-10294 / GST Bill / Cash memo"
              />
            </div>
          </div>

          {/* Expense Audit & Compliance Information Box */}
          <div className="p-3.5 rounded-xl bg-slate-50 border border-slate-200/80 space-y-2 text-xs">
            <div className="flex items-center justify-between text-slate-800 font-bold text-[11px]">
              <span className="flex items-center gap-1.5 text-indigo-700">
                <Receipt className="w-3.5 h-3.5 text-indigo-600" />
                Audit & Voucher Reconciliation
              </span>
              <span className="text-[10px] font-mono text-emerald-700 bg-emerald-50 border border-emerald-200/80 px-2 py-0.5 rounded-full font-semibold">
                Auto GL Voucher
              </span>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-[11px] text-slate-600">
              <div className="flex items-start gap-1.5">
                ...
                <span><strong>Wallet Debit:</strong> Amount is debited immediately from your active {formData.fundMode === 'LIQUID' ? 'liquid' : 'cash'} balance.</span>
              </div>
              <div className="flex items-start gap-1.5">
                <CheckCircle2 className="w-3 h-3 text-indigo-600 mt-0.5 shrink-0" />
                <span><strong>Tax & Audit:</strong> Retain original GST bill or vendor voucher for periodic internal audit.</span>
              </div>
            </div>
          </div>
