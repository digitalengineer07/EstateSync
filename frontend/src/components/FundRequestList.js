...
export default function FundRequestList({ type = "outgoing", embedded = false, showHeader = true }) {
  ...
  return (
    <div className={embedded ? "space-y-4" : "bg-white rounded-2xl sm:rounded-[22px] border border-slate-200/90 shadow-[0_4px_24px_-6px_rgba(0,0,0,0.04)] p-6 sm:p-7 space-y-5"}>
      ...
      {error && data && (
        <div className="p-2.5 bg-orange-50 text-orange-800 border border-orange-200 rounded-xl text-xs flex items-center justify-between">
          <span>⚠️ Disconnected - Retrying...</span>
        </div>
      )}

      {actionSuccess && (
        <div className="p-3.5 bg-orange-50 text-orange-800 border border-orange-200 rounded-xl text-xs flex items-center justify-between">
          <div className="flex items-center gap-2">
            <CheckCircle2 className="w-4 h-4 text-orange-600 shrink-0" />
            <span>{actionSuccess}</span>
          </div>
          <button onClick={() => setActionSuccess(null)} className="text-orange-600 hover:text-orange-900 text-xs font-bold ml-4">✕</button>
        </div>
      )}
      ...
      {actionError && (
        <div className="p-3.5 bg-orange-50 text-orange-800 border border-orange-200 rounded-xl text-xs flex items-center justify-between">
          ...
          <button onClick={() => setActionError(null)} className="text-orange-600 hover:text-orange-900 text-xs font-bold ml-4">✕</button>
        </div>
      )}
                  ...
                  <tr key={req.id} className="hover:bg-slate-50/70 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <span className={`px-2.5 py-0.5 inline-flex text-[10.5px] font-bold rounded-md border 
                        ${req.status === 'APPROVED' ? 'bg-orange-50 text-orange-700 border-orange-200' : 
                          req.status === 'REJECTED' ? 'bg-orange-50 text-orange-700 border-orange-200' : 
                        ...
                        {req.status === 'PENDING' ? (
                          <div className="flex justify-end items-center space-x-2">
                            <select
                              ...
                              className="px-2 py-1 bg-white border border-slate-200 rounded-lg text-[10px] font-bold text-slate-700 outline-none focus:ring-2 focus:ring-orange-500"
                            >
                            ...
                            <button 
                              ...
                              className="px-3 py-1 bg-orange-600 hover:bg-orange-700 text-white rounded-lg text-xs font-semibold transition-colors disabled:opacity-50 active:scale-95"
                            >
                            ...
                            <button 
                              ...
                              className="px-3 py-1 bg-orange-50 hover:bg-orange-100 text-orange-700 rounded-lg text-xs font-semibold border border-orange-200 transition-colors disabled:opacity-50 active:scale-95"
                            >
                              Reject
                            </button>
                          </div>
