        ...
        <div className="overflow-x-auto rounded-lg border border-slate-200">
          <table className="min-w-full divide-y divide-slate-200 text-left text-xs whitespace-nowrap">
            <thead className="bg-slate-50/90 font-semibold text-slate-500 uppercase tracking-wider text-[11px]">
              <tr>
                <th className="px-3.5 py-2.5">Khata & Plot</th>
                <th className="px-3.5 py-2.5">Land Owner</th>
                <th className="px-3.5 py-2.5 text-right">Total Land Value</th>
                <th className="px-3.5 py-2.5">Payout Progress</th>
            ...
            <tbody className="divide-y divide-slate-100 bg-white">
              {filteredProperties.map((prop) => {
                ...
                const displayPlot = String(prop.plotNo || "").replace(/[-_ ]*(CORRECT|CORRECTED|VERIFIED)/gi, "").trim();
                const displayKhata = String(prop.khataNo || "").replace(/[-_ ]*(VERIFIED|VERIFY)/gi, "").trim();
                ...
                return (
                  ...
                  >
                    <td className="px-3.5 py-2.5">
                      <div className="font-bold text-slate-900 leading-tight">Plot {displayPlot}</div>
                      <div className="text-[11px] text-slate-500 leading-tight mt-0.5">{prop.projectLocation}</div>
                      <div className="text-[10px] text-orange-600 font-mono leading-tight mt-0.5">Khata: {displayKhata} • {prop.areaDescription || "N/A"}</div>
                    </td>

                    <td className="px-3.5 py-2.5">
                      <div className="font-semibold text-slate-900 leading-tight">{prop.landOwnerName}</div>
                      <div className="text-[11px] text-slate-500 leading-tight mt-0.5">{prop.landOwnerContact}</div>
                      <div className="text-[10px] text-slate-400 leading-tight mt-0.5">Owner A/C: {prop.ownerBankAccount || "Direct"}</div>
                    </td>

                    <td className="px-3.5 py-2.5 text-right font-mono">
                      <div className="font-bold text-slate-900 text-sm leading-tight">₹{total.toLocaleString('en-IN')}</div>
                      <div className="text-[10px] text-slate-400 font-sans leading-tight mt-0.5">Agreement: {new Date(prop.agreementDate).toLocaleDateString()}</div>
                    </td>
                    ...
                    <td className="px-3.5 py-2.5 min-w-[150px]">
                      ...
                      <div className="text-[10px] text-slate-400 text-right mt-0.5 font-medium leading-tight">{pct}% Disbursed</div>
                    </td>
