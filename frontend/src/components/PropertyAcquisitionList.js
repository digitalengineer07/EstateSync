                ...
                return (
                  <tr key={prop.id} className="hover:bg-slate-50/70 transition-colors">
                    ...
                    <td className="px-4 py-3.5 text-right font-mono">
                      <div className="font-bold text-slate-900 text-sm">₹{total.toLocaleString()}</div>
                    ...
                    <td className="px-4 py-3.5 min-w-[160px]">
                      <div className="flex justify-between text-[11px] font-semibold mb-1 font-mono">
                        ...
                        <span className="text-slate-500">Rem: ₹{balance.toLocaleString()}</span>
                      </div>
                    ...
                    <td className="px-4 py-3.5 text-right space-x-2">
                      ...
                      {canManage && prop.status !== 'FULLY_PAID' && balance > 0 && (
                        <button
                          onClick={() => handleOpenPayment(prop)}
                          className="px-3 py-1 text-[11px] font-bold text-white bg-indigo-600 hover:bg-indigo-700 rounded-md shadow-xs transition active:scale-95"
                        >
                          + Record Payout
                        </button>
                      )}
          ...
          <div className="bg-white rounded-xl shadow-2xl max-w-2xl w-full flex flex-col overflow-hidden border border-slate-200">
            <div className="px-6 py-4 bg-slate-900 text-white flex justify-between items-center">
              <div>
                ...
                <p className="text-xs text-slate-400">Khata {historyProperty.khataNo}, Plot {historyProperty.plotNo} ({historyProperty.projectLocation}) • Valuation: ₹{parseFloat(historyProperty.totalLandValue).toLocaleString()}</p>
              </div>
              <button
                  ...
                  <tbody className="divide-y divide-slate-100 font-mono">
                    {historyProperty.payments.map((p) => (
                      <tr key={p.id} className="hover:bg-slate-50">
                        ...
                        <td className="px-3 py-2 text-right font-bold text-rose-600">₹{parseFloat(p.amount).toLocaleString()}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
