              ...
              <tbody className="divide-y divide-slate-100 text-slate-800 font-normal">
                {expenses.map((item) => (
                  <tr key={item.id} className="hover:bg-slate-50/70 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <span className="px-2.5 py-0.5 text-[11px] font-semibold bg-orange-50 text-orange-700 rounded-md border border-orange-200/80">
                      ...
                      <span
                        className={`px-2.5 py-0.5 text-[10.5px] font-bold rounded-md border ${
                          item.status === "RECORDED"
                            ...
                            : "bg-orange-50 text-orange-700 border-orange-200 line-through"
                        }`}
                      >
                        {item.status}
                      </span>
                      ...
                      <td className="px-4 py-3 text-right">
                        {item.status === "RECORDED" ? (
                          <button
                            ...
                            className="px-2.5 py-1 text-xs font-semibold text-orange-700 bg-orange-50 hover:bg-orange-100 border border-orange-200 rounded-lg transition active:scale-95"
                          >
                            Reverse
                          </button>
                        ) : (
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-xl shadow-2xl max-w-md w-full p-6 border border-gray-100">
            ...
            <p className="text-sm text-gray-600 mt-2">
              ...
              <strong className="text-orange-600">₹{parseFloat(selectedExpense.amount).toLocaleString('en-IN')}</strong> recorded by{" "}
              <strong>{selectedExpense.user?.name || "User"}</strong>?
            </p>
            <div className="mt-3 p-3 bg-orange-50 rounded-md border border-orange-200 text-xs text-orange-900">
            ...
            <form onSubmit={handleReverseExpense} className="mt-4 space-y-3">
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">
                  Reason for Reversal <span className="text-orange-500">*</span>
                </label>
                <input
                  type="text"
                  required
              ...
              <div className="flex justify-end space-x-3 pt-2">
                ...
                <button
                  ...
                  className="px-4 py-2 text-xs font-bold text-white bg-orange-600 hover:bg-orange-700 rounded-md transition-colors disabled:opacity-50"
                >
